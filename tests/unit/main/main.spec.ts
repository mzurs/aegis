...
describe("======================================Main Canister Unit Testing======================================", () => {
  let pic: PocketIc;
  let minter: Identity;
  let user1: Identity;
  let user2: Identity;
  ...
  beforeAll(async () => {
    ...
    mainActor.set_canister_id(
      { AEGIS: null },
      CANISTER_IDS_MAP.get(CANISTERS_NAME.AEGIS_LEDGER)!
    );

    mainActor.set_min_staking_delay([60n]);
  });
  ...
  describe("Aegis Transfer Test", () => {
    ...
    it("Transfer 1000 AEGIS tokens to Main canister wallet", async () => {
      ...
      let res = await transferTokens(
        ...
        await mainActor.convert_u32_to_subaccount(1)
      );
  ...
  describe("Staking Test", () => {
    it("Staking delay should be 60 sec", async () => {
      let delay = await mainActor.get_min_staking_delay();
      expect(delay).toBe(60n * 1000000000n);
    });

    it("TVL should be 0", async () => {
      let tvl: TotalValueLockedRes = await mainActor.get_total_value_locked({
        ICRC: {
          AEGIS: null,
        },
      });
    ...
    it("Stake 10 tokens with user1 Identity", async () => {
      mainActor.setIdentity(user1);
      let res: StakeIcrcRes = await mainActor.icrc_stake_tokens(
        { AEGIS: null },
    ...
    it("Manual UnStake tokens with User1 Identity  ", async () => {
      ...
      let res: ExecuteUnstakeAmountRes =
        await mainActor.icrc_unstake_tokens_manual({
          AEGIS: null,
        });
    ...
    it("Set Rewards Duration", async () => {
      mainActor.set_rewards_duration({ ICRC: { AEGIS: null } }, 30n);
