...
describe("Options Canister Integration Testing", () => {
  ...
  describe("Options Main Methods 4", () => {
    ...
    it("Create CALL Option Contract of 0.00001 CkETH for user1 ", async () => {
      let actor = createCanisterActor(
        CANISTERS_NAME_NO_PIC.OPTIONS,
        user1
      ) as unknown as ActorSubclass<_OPTIONS>;
      ...
      let res = await actor.create_icrc_options(
        ...
        },
        {
          ...
          strike_price: 2000n * BASE_OF_XRC,
      ...
      ).toBeLessThan(parseEther("0.00001"));
    ...
    it("Trade CALL Option Contract with user2 Id should succeed", async () => {
    ...
    it("Execute Option Automatically should succeeded", async () => {
      ...
      expect(
        await balance(CANISTERS_NAME_NO_PIC.CKETH_LEDGER, user1.getPrincipal())
      ).toBeGreaterThan(balanceCkethUser1);

      expect(
        await balance(CANISTERS_NAME_NO_PIC.CKETH_LEDGER, user2.getPrincipal())
      ).toBeGreaterThan(balanceCkethUser2);

      expect(
        await balance(CANISTERS_NAME_NO_PIC.CKUSDT_LEDGER, user1.getPrincipal())
      ).toBe(balanceCkUsdtUser1);
